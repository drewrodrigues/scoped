...
export abstract class CouchModel<T> {
  ...
  constructor(attributes: UnsavedConstructor<T> | SavedConstructor<T>) {
    ...
    if (this.isSavedAttributes(attributes)) {
      console.log(`ℹ️ Init saved ${this.__type} record`);
      this._id = attributes._id;
      this.__isDirty = false;
    } else {
      console.log(`ℹ️ Init new ${this.__type} record`);
      this._id = `${this.__type}-${Date.now().toString()}`;
      this.__isDirty = true;
    }
  ...
  public async save(parentRecord?: SavedType<any>) {
    ...
    const attributes = this.savableAttributes();
    ...
    try {
      console.info(`ℹ️ Creating ${newOrOld} ${this.__type} record`, {
        attributes,
      });
      const createResponse = await db.put(attributes);
      const createdRecord = { _rev: createResponse.rev, ...attributes };
      console.info(`✅ ${this.__type} created`, {
        createResponse,
        createdRecord,
      });
      this.__isDirty = false;
